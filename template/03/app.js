(() => {
  ...
  try {
    ...
    if (defenseNode) {
      ...
      const rows = [
        { label: '开题通过', value: Number(defense.pass || 0), color: 'var(--jade)' },
        { label: '未通过', value: Number(defense.fail || 0), color: 'var(--red)' },
        { label: '修改后通过', value: Number(defense.revised || 0), color: 'var(--gold)' },
        { label: '博资考通过', value: Number(defense.exam || 0), color: 'var(--indigo)' },
      ]
